...
function PlanEditor({ plan, featureCatalog, onDone }: { plan: any | null; featureCatalog: readonly any[]; onDone: () => void }) {
  ...
  return <section className={`rounded-2xl border bg-slate-900 p-5 shadow-xl ${plan ? 'border-slate-800' : 'border-indigo-700'}`}>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3"><label className="text-xs font-bold text-slate-400">Precio mensual<input type="number" min="0" value={draft.priceMonthly} onChange={(e) => setDraft({ ...draft, priceMonthly: e.target.value })} className={fieldClass} /></label><label className="text-xs font-bold text-slate-400">Precio anual<input type="number" min="0" value={draft.priceYearly} onChange={(e) => setDraft({ ...draft, priceYearly: e.target.value })} className={fieldClass} /></label></div>
...
function Limit({ icon: Icon, label, value, set }: { icon: any; label: string; value: string | number; set: (value: string) => void }) {
  return <label className="text-[10px] font-bold text-slate-500"><span className="flex items-center gap-1"><Icon className="h-3 w-3" />{label}</span><input type="number" min="0" value={value} onChange={(e) => set(e.target.value)} className={fieldClass} /></label>;
}
