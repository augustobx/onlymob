...
export default async function ContratosPage(){
 ...
 const availableProperties=properties.filter(p=>p.status==='DISPONIBLE');const availableSpaces=garages.flatMap(g=>g.spaces.filter(s=>s.status==='FREE').map(s=>({id:s.id,spaceNumber:s.spaceNumber,garageName:g.name})));
 return <div><Header title="Contratos" subtitle="Locaciones, garantías, ajustes y cuenta contractual"/><main className="app-page"><div className="page-container space-y-6">
