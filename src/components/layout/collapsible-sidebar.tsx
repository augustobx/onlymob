...
type NavGroup = { label:string; items:Array<{name:string;href:string;icon:any}> };
...
export function CollapsibleSidebar({tenantName,userName}:SidebarProps){
 ...
 const activeGroup=useMemo(()=>groups.find(group=>group.items.some(item=>pathname===item.href||pathname.startsWith(`${item.href}/`)))?.label||'General',[pathname]);
 const [openGroups,setOpenGroups]=useState<Record<string,boolean>>(()=>Object.fromEntries(groups.map(group=>[group.label,group.label==='General'||group.label===activeGroup])));
 ...
 return <aside className="app-sidebar">
  ...
  <nav className="app-sidebar__nav">{groups.map(group=>{const open=openGroups[group.label]||group.label===activeGroup;return <div key={group.label} className="app-sidebar__group">
    ...
    {open&&<div className="space-y-1">{group.items.map(item=>{const active=pathname===item.href||pathname.startsWith(`${item.href}/`);const Icon=item.icon;return <Link key={item.href} href={item.href} className={`app-sidebar__link ${active?'is-active':''}`}><Icon className="w-[17px] h-[17px]"/><span>{item.name}</span></Link>})}</div>}
  </div>})}</nav>
