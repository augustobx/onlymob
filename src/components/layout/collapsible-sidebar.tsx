...
interface SidebarProps { tenantName:string; userName:string; features?: Record<string, boolean>; }
type NavItem={name:string;href:string;icon:any;quick?:boolean;feature?:'analytics'|'integrations'};
...
const groups:NavGroup[]=[
 {label:'General',items:[{name:'Dashboard',href:'/dashboard',icon:LayoutDashboard},{name:'Analytics',href:'/analytics',icon:ChartNoAxesCombined,feature:'analytics'}]},
 ...
 {label:'Sistema',items:[{name:'Integraciones',href:'/integraciones',icon:Plug,feature:'integrations'},{name:'Configuración',href:'/ajustes',icon:Settings}]},
];

export function CollapsibleSidebar({tenantName,userName,features={}}:SidebarProps){
 const pathname=usePathname();
 const visibleGroups=useMemo(()=>groups.map(group=>({...group,items:group.items.filter(item=>!item.feature||features[item.feature]!==false)})).filter(group=>group.items.length>0),[features]);
 const activeGroup=useMemo(()=>visibleGroups.find(group=>group.label!=='Rápidos'&&group.items.some(item=>matches(pathname,item)))?.label||'General',[pathname,visibleGroups]);
 ...
 return <aside className="app-sidebar">
  ...
  <nav className="app-sidebar__nav">{visibleGroups.map(group=>{const open=openGroups[group.label]||group.label===activeGroup;return <div key={group.label} className="app-sidebar__group">
