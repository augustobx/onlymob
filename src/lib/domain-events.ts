...
const ACTION_EVENTS: Record<string, EventDescriptor> = {
  ...
  LEAD_INTERACTION_CREATED: { eventKey: 'lead.updated', title: 'Interacción comercial registrada' },
  INTERACTION_CREATED: { eventKey: 'lead.updated', title: 'Interacción comercial registrada' },

  EVENT_CREATED: { eventKey: 'visit.created', title: 'Evento/visita agendada' },
  EVENT_UPDATED: { eventKey: 'visit.created', title: 'Evento/visita actualizada' },
  ...
  DEAL_UPDATED: { eventKey: 'deal.updated', title: 'Operación actualizada' },
  ...
  LEASE_INCREASE_APPLIED: { eventKey: 'lease.updated', title: 'Alquiler actualizado' },
  LEASE_TERMINATED: { eventKey: 'lease.updated', title: 'Contrato finalizado' },
  ...
  PAYMENT_RECORDED: { eventKey: 'payment.registered', title: 'Pago registrado' },
  ...
  DOCUMENT_REGISTERED: { eventKey: 'document.created', title: 'Documento registrado' },
  ...
  OWNER_SETTLEMENT_CREATED: { eventKey: 'settlement.ready', title: 'Liquidación disponible' },
  SETTLEMENT_CREATED: { eventKey: 'settlement.ready', title: 'Liquidación disponible' },
...
function fallbackDescriptor(input: DomainActivityInput): EventDescriptor {
  ...
  return {
    ...
    title: input.action.toLowerCase().replaceAll('_', ' ').replace(/^./, (char) => char.toUpperCase()),
  };
}

function descriptorFor(input: DomainActivityInput): EventDescriptor {
  if (input.action === 'DEAL_STATUS_CHANGED') {
    return input.metadata?.status === 'WON'
      ? { eventKey: 'deal.won', title: 'Operación ganada' }
      : { eventKey: 'deal.updated', title: 'Estado de operación actualizado' };
  }

  if (input.action === 'OWNER_SETTLEMENT_STATUS_CHANGED' || input.action === 'SETTLEMENT_STATUS_CHANGED') {
    return input.metadata?.status === 'READY'
      ? { eventKey: 'settlement.ready', title: 'Liquidación disponible' }
      : { eventKey: 'settlement.updated', title: 'Liquidación actualizada' };
  }

  return ACTION_EVENTS[input.action] || fallbackDescriptor(input);
}
...
async function resolveLinks(input: DomainActivityInput) {
  ...
  if ((!propertyId || !contactId || !renterId) && entityId) {
    if (input.entityType === 'Lead') {
      const row = await platformPrisma.lead.findFirst({
        where: { id: entityId, tenantId: input.tenantId },
        select: { contactId: true, propertyInterests: { take: 1, select: { propertyId: true } } },
      });
      contactId ||= row?.contactId || null;
      propertyId ||= row?.propertyInterests[0]?.propertyId || null;
    } else if (input.entityType === 'Reservation') {
      const row = await platformPrisma.reservation.findFirst({
        where: { id: entityId, tenantId: input.tenantId },
      ...
      const row = await platformPrisma.deal.findFirst({
        where: { id: entityId, tenantId: input.tenantId },
        select: { propertyId: true, contactId: true },
      });
      propertyId ||= row?.propertyId || null;
      ...
      const row = await platformPrisma.publication.findFirst({
        where: { id: entityId, tenantId: input.tenantId },
        select: { propertyId: true },
      });
      propertyId ||= row?.propertyId || null;
    } else if (input.entityType === 'PropertyLease') {
      const row = await platformPrisma.propertyLease.findFirst({
        where: { id: entityId, tenantId: input.tenantId },
        select: { propertyId: true, renterId: true },
      });
      propertyId ||= row?.propertyId || null;
      renterId ||= row?.renterId || null;
    } else if (input.entityType === 'Debt') {
      const row = await platformPrisma.debt.findFirst({
        where: { id: entityId, tenantId: input.tenantId },
        select: { renterId: true, propertyLease: { select: { propertyId: true } } },
      });
      renterId ||= row?.renterId || null;
      propertyId ||= row?.propertyLease?.propertyId || null;
    } else if (input.entityType === 'Payment') {
      const row = await platformPrisma.payment.findFirst({
        where: { id: entityId, tenantId: input.tenantId },
        select: { debt: { select: { renterId: true, propertyLease: { select: { propertyId: true } } } } },
      });
      ...
      const row = await platformPrisma.maintenanceRequest.findFirst({
        where: { id: entityId, tenantId: input.tenantId },
        select: { propertyId: true, renterId: true },
      });
      propertyId ||= row?.propertyId || null;
      ...
      const row = await platformPrisma.ownerSettlement.findFirst({
        where: { id: entityId, tenantId: input.tenantId },
...
export async function publishDomainActivity(input: DomainActivityInput) {
  ...
  if (input.entityType === 'Payment' && input.entityId && ['PAYMENT_RECORDED', 'PAYMENT_REGISTERED', 'PAYMENT_CREATED'].includes(input.action)) {
    try {
      await projectPaymentToFinance(input.tenantId, input.entityId);
    } catch (error) {
      console.error('[OnlyMob finance-projection]', error);
    }
  }
  ...
  if (queued.queued > 0) {
    await platformPrisma.$executeRaw(Prisma.sql`
      UPDATE ActivityEvent SET webhookQueuedAt=${new Date()} WHERE id=${id}
    `);
  }
