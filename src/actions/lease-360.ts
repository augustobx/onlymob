...
import { leaseAutoSettingKey } from '@/lib/rent-adjustment-engine';
...
export async function getLease360Action(leaseId:string){
 ...
 const[maintenance,inspections,activity,financial,autoSetting]=await Promise.all([
  ...
  platformPrisma.tenantSetting.findUnique({where:{tenantId_key:{tenantId:tenant.id,key:leaseAutoSettingKey(lease.id)}}}),
 ]);
 return JSON.parse(JSON.stringify({lease,maintenance,inspections,activity,financial,autoAdjustmentEnabled:autoSetting?.value==='true'}));
}
