...
export async function getOperation360Action(dealId:string){
 ...
 const deal=await platformPrisma.deal.findFirst({where:{id:dealId,tenantId:tenant.id},include:{
  ...
  contact:true,lead:{include:{interactions:{orderBy:{occurredAt:'desc'}},propertyInterests:{include:{property:{select:{id:true,code:true,address:true}}},orderBy:{updatedAt:'desc'}}}},agent:{select:{id:true,name:true,email:true}},reservation:true,propertyLease:{include:{renter:true,debts:{include:{payments:true}}}},documents:{orderBy:{uploadedAt:'desc'}},
 }});if(!deal)return null;
 ...
 return JSON.parse(JSON.stringify({deal,activity,communications}));
}
