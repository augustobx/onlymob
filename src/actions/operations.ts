...
function calculateCompleteness(property: {
  ...
  const checks = [
    ...
    property.sqm != null,
    property.operation === 'SALE' ? property.salePrice != null : property.rentPrice != null,
...
export async function savePublicationAction(data: { id?: string; propertyId: string; channel: string; status?: typeof PUBLICATION_STATUSES[number]; url?: string; notes?: string }) {
  ...
  if (!data.channel.trim()) throw new Error('Indicá el canal de publicación.');
  const property = await assertProperty(tenant.id, data.propertyId);
  const status = data.status || 'DRAFT';
  const completenessScore = calculateCompleteness(property);
  const payload = {
    ...
    unpublishedAt: status === 'ENDED' ? new Date() : null,
  };

  let publication;
  if (data.id) {
    const existing = await platformPrisma.publication.findFirst({ where: { id: data.id, tenantId: tenant.id } });
    if (!existing) throw new Error('Publicación no encontrada.');
    publication = await platformPrisma.publication.update({ where: { id: existing.id }, data: payload });
  } else {
    publication = await platformPrisma.publication.upsert({
      where: { tenantId_propertyId_channel: { tenantId: tenant.id, propertyId: property.id, channel: data.channel.trim() } },
      update: payload,
      create: { tenantId: tenant.id, ...payload },
    });
  }
...
const ReservationSchema = z.object({
  ...
  status: z.enum(RESERVATION_STATUSES).default('PENDING'),
  expiresAt: z.string().optional().nullable(),
  conditions: z.string().max(6000).optional().nullable(),
...
export async function saveReservationAction(data: z.input<typeof ReservationSchema> & { id?: string }) {
  ...
  const [property, contact] = await Promise.all([assertProperty(tenant.id, validated.propertyId), assertContact(tenant.id, validated.contactId)]);
  await Promise.all([assertLead(tenant.id, validated.leadId), assertAgent(tenant.id, validated.agentId)]);
  ...
  const reservation = await platformPrisma.$transaction(async (tx) => {
    const payload = {
      ...
      expiresAt: optionalDate(validated.expiresAt),
      conditions: validated.conditions?.trim() || null,
      commissionAmount: validated.commissionAmount ?? null,
      notes: validated.notes?.trim() || null,
      convertedAt: validated.status === 'CONVERTED' ? new Date() : null,
    };
    let saved;
    if (data.id) {
      const existing = await tx.reservation.findFirst({ where: { id: data.id, tenantId: tenant.id } });
      if (!existing) throw new Error('Reserva no encontrada.');
      saved = await tx.reservation.update({ where: { id: existing.id }, data: payload });
    } else saved = await tx.reservation.create({ data: { tenantId: tenant.id, ...payload } });
  ...
  revalidatePath('/operaciones'); revalidatePath('/crm'); revalidatePath('/propiedades'); revalidatePath('/dashboard');
  return { success: true, reservationId: reservation.id };
}

export async function setReservationStatusAction(reservationId: string, status: typeof RESERVATION_STATUSES[number]) {
  const { tenant, session } = await requireTenantAdmin();
  const reservation = await platformPrisma.reservation.findFirst({ where: { id: reservationId, tenantId: tenant.id }, include: { deal: true } });
  if (!reservation) throw new Error('Reserva no encontrada.');
  if (reservation.deal && status !== 'CONVERTED') throw new Error('La reserva ya fue convertida en operación.');
  await platformPrisma.$transaction(async (tx) => {
    await tx.reservation.update({ where: { id: reservation.id }, data: { status, convertedAt: status === 'CONVERTED' ? new Date() : null } });
    if (['CANCELED','EXPIRED'].includes(status)) {
      const active = await tx.reservation.count({ where: { tenantId: tenant.id, propertyId: reservation.propertyId, id: { not: reservation.id }, status: { in: ['PENDING','CONFIRMED'] } } });
      if (active === 0) await tx.property.update({ where: { id: reservation.propertyId }, data: { commercialStatus: 'AVAILABLE' } });
    }
  });
  await auditTenantAction({ tenantId: tenant.id, actorUserId: session.userId, action: 'RESERVATION_STATUS_CHANGED', entityType: 'Reservation', entityId: reservation.id, metadata: { status } });
  revalidatePath('/operaciones'); revalidatePath('/crm'); revalidatePath('/propiedades');
  return { success: true };
}

export async function convertReservationToDealAction(reservationId: string) {
  const { tenant, session } = await requireTenantAdmin();
  const reservation = await platformPrisma.reservation.findFirst({ where: { id: reservationId, tenantId: tenant.id }, include: { property: true, deal: true } });
  if (!reservation) throw new Error('Reserva no encontrada.');
  if (reservation.deal) return { success: true, dealId: reservation.deal.id };
  if (['CANCELED','EXPIRED'].includes(reservation.status)) throw new Error('No se puede convertir una reserva cancelada o vencida.');

  const deal = await platformPrisma.$transaction(async (tx) => {
    const created = await tx.deal.create({ data: { tenantId: tenant.id, propertyId: reservation.propertyId, contactId: reservation.contactId, leadId: reservation.leadId, reservationId: reservation.id, agentId: reservation.agentId, operation: reservation.property.operation, status: 'NEGOTIATION', amount: reservation.amount, currency: reservation.currency, commissionAmount: reservation.commissionAmount, notes: reservation.notes } });
    await tx.reservation.update({ where: { id: reservation.id }, data: { status: 'CONVERTED', convertedAt: new Date() } });
    await tx.property.update({ where: { id: reservation.propertyId }, data: { commercialStatus: 'UNDER_NEGOTIATION' } });
    if (reservation.leadId) await tx.lead.update({ where: { id: reservation.leadId }, data: { status: 'NEGOTIATION' } });
    return created;
  });

  await auditTenantAction({ tenantId: tenant.id, actorUserId: session.userId, action: 'RESERVATION_CONVERTED_TO_DEAL', entityType: 'Deal', entityId: deal.id, metadata: { reservationId } });
  revalidatePath('/operaciones'); revalidatePath('/crm'); revalidatePath('/propiedades'); revalidatePath('/dashboard');
...
export async function saveDealAction(data: z.input<typeof DealSchema> & { id?: string }) {
  ...
  if (validated.contactId) await assertContact(tenant.id, validated.contactId);
  await Promise.all([assertLead(tenant.id, validated.leadId), assertAgent(tenant.id, validated.agentId)]);
  ...
  let deal;
  if (data.id) {
    const existing = await platformPrisma.deal.findFirst({ where: { id: data.id, tenantId: tenant.id } });
    if (!existing) throw new Error('Operación no encontrada.');
    deal = await platformPrisma.deal.update({ where: { id: existing.id }, data: payload });
  } else deal = await platformPrisma.deal.create({ data: { tenantId: tenant.id, ...payload } });
  await auditTenantAction({ tenantId: tenant.id, actorUserId: session.userId, action: data.id ? 'DEAL_UPDATED' : 'DEAL_CREATED', entityType: 'Deal', entityId: deal.id });
  revalidatePath('/operaciones'); revalidatePath('/crm'); revalidatePath('/dashboard');
  return { success: true, dealId: deal.id };
}

export async function setDealStatusAction(dealId: string, status: typeof DEAL_STATUSES[number]) {
  const { tenant, session } = await requireTenantAdmin();
  const deal = await platformPrisma.deal.findFirst({ where: { id: dealId, tenantId: tenant.id } });
  if (!deal) throw new Error('Operación no encontrada.');
  const now = new Date();
  await platformPrisma.$transaction(async (tx) => {
    await tx.deal.update({ where: { id: deal.id }, data: { status, closedAt: ['WON','LOST','CANCELED'].includes(status) ? now : null } });
    if (status === 'WON') {
      await tx.property.update({ where: { id: deal.propertyId }, data: { commercialStatus: 'CLOSED' } });
      if (deal.leadId) await tx.lead.update({ where: { id: deal.leadId }, data: { status: 'WON', closedAt: now } });
    } else if (['LOST','CANCELED'].includes(status)) {
      await tx.property.update({ where: { id: deal.propertyId }, data: { commercialStatus: 'AVAILABLE' } });
      if (deal.leadId && status === 'LOST') await tx.lead.update({ where: { id: deal.leadId }, data: { status: 'LOST', closedAt: now, lostReason: 'Operación perdida' } });
    } else await tx.property.update({ where: { id: deal.propertyId }, data: { commercialStatus: 'UNDER_NEGOTIATION' } });
  });
  await auditTenantAction({ tenantId: tenant.id, actorUserId: session.userId, action: 'DEAL_STATUS_CHANGED', entityType: 'Deal', entityId: deal.id, metadata: { status } });
  revalidatePath('/operaciones'); revalidatePath('/crm'); revalidatePath('/propiedades'); revalidatePath('/dashboard');
  return { success: true };
}
...
export async function linkDealToLeaseAction(dealId: string, propertyLeaseId: string) {
  const { tenant, session } = await requireTenantAdmin();
  const [deal, lease] = await Promise.all([
    platformPrisma.deal.findFirst({ where: { id: dealId, tenantId: tenant.id } }),
    platformPrisma.propertyLease.findFirst({ where: { id: propertyLeaseId, tenantId: tenant.id } }),
  ]);
  if (!deal) throw new Error('Operación no encontrada.');
  if (!lease) throw new Error('Contrato no encontrado.');
  if (deal.propertyId !== lease.propertyId) throw new Error('El contrato no corresponde a la propiedad de la operación.');
  await platformPrisma.deal.update({ where: { id: deal.id }, data: { propertyLeaseId: lease.id, status: 'WON', closedAt: new Date() } });
  await auditTenantAction({ tenantId: tenant.id, actorUserId: session.userId, action: 'DEAL_LINKED_TO_LEASE', entityType: 'Deal', entityId: deal.id, metadata: { propertyLeaseId: lease.id } });
  revalidatePath('/operaciones'); revalidatePath('/contratos');
  return { success: true };
}
